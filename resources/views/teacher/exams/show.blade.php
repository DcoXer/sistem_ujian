                    ...
                    @forelse ($exam->questions as $question)
                        ...
                        <div class="rounded-lg border border-gray-200 p-3">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                @if ($canManageQuestions)
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('teacher.exams.questions.destroy', [$exam, $question]) }}" data-confirm data-confirm-title="Hapus Soal" data-confirm-message="Soal ini akan dihapus. Lanjutkan?">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @if ($question->question_image_path)
                                <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($question->question_image_path) }}" alt="Gambar soal" class="mt-3 max-h-56 rounded-lg border border-slate-200 object-contain">
                            @endif
                            ...
                            <div id="edit-question-modal-{{ $question->id }}" class="fixed inset-0 z-[106] hidden">
                                ...
                                <div class="relative z-10 flex min-h-full items-center justify-center p-4">
                                    <div class="w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
                                        ...
                                        <form method="POST" action="{{ route('teacher.exams.questions.update', [$exam, $question]) }}" enctype="multipart/form-data" class="space-y-3">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div>
                                                <label class="mb-1 block text-sm font-medium text-gray-700">Gambar Soal (opsional)</label>
                                                <input type="file" name="question_image" accept="image/*" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
        ...
        <div id="question-modal" class="fixed inset-0 z-[105] hidden">
            ...
            <div class="relative z-10 flex min-h-full items-center justify-center p-4">
                <div class="w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
                    ...
                    <form method="POST" action="{{ route('teacher.exams.questions.store', $exam) }}" enctype="multipart/form-data" class="space-y-3">
                        @csrf
                        ...
                        <div>
                            <label class="mb-1 block text-sm font-medium text-gray-700">Gambar Soal (opsional)</label>
                            <input type="file" name="question_image" accept="image/*" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm">
                            <p class="mt-1 text-xs text-gray-500">Boleh kosong kalau soal tidak pakai gambar.</p>
