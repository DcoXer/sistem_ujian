<section id="mulai-ujian" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    @if ($activeExam)
        ...
        <a href="{{ route('student.exams.index') }}" class="mt-4 inline-flex rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">
            Mulai / Lanjut Ujian
        </a>
    @else
...
<section id="jawab-soal" class="grid gap-4 lg:grid-cols-2">
    <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        @if ($myInProgressAttempt)
            ...
            <a href="{{ route('student.exams.show', $myInProgressAttempt->id) }}" class="mt-4 inline-flex rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700">
                Lanjut Kerjakan
            </a>
        @else
            <p class="mt-2 text-sm text-slate-600">Belum ada attempt aktif untuk saat ini.</p>
            <a href="{{ route('student.exams.index') }}" class="mt-4 inline-flex rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">
                Buka Daftar Ujian
            </a>
        @endif
    </article>
